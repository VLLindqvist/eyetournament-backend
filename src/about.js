...
class About extends Library {
    ...
    async index(){
      ...
      if(this.method == 'POST'){this.update(); return;}
    ...
    async update() {
      ...
      if(this.isset(input, ["content", "lang"])) {
        let data = await this.db.find('main', {type: "about"}, {about: 1, _id: 0});
        const content = await new Content("about", data, input.lang, input.content);
        const newContent = await content.getContent();
